...
class UserProfile(TemplateView):
    ...
    main_header = _('Профиль{username}')
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            if form.is_valid():
                ...
                context['msg'] = _('Профиль успешно обновлён')
                context['msg_theme'] = 'success'
        
        else:
            context['msg'] = _('Неавторизованным пользователям нет доступа к редактированию профиля')
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if not captcha.isdigit():
                form.add_error('captcha', _('Решение должно быть числом!'))
            else:
                ...
                if captcha == captcha_session:
                    ...
                    if new_user.id:
                        ...
                        if new_profile.id:
                            ...
                            form.add_error(None, _('Проблема с добавлением профиля в БД'))
                    else:
                        form.add_error(None, _('Проблема с добавлением пользователя в БД'))
                
                else:
                    form.add_error('captcha', _('Пример решен неверно!'))
...
# class UserVerify(TemplateView):
#     template_name = 'pages/moder/verify_users.html'
#
#     main_header = 'Верификация'
#     page_title = f'{main_header}'
#
#     def get_context_data(self, **kwargs):
#         """
...
#         context['page_title'] = self.page_title
#
#         unverified_users = User.objects.filter(groups__name='Зарегистрированные пользователи')
#
...
#         if user.id:
#             group_unv = Group.objects.get(name='Зарегистрированные пользователи')
#             user.groups.remove(group_unv)
#             group_v = Group.objects.get(name='Верифицированные пользователи')
#             group_v.user_set.add(user)
